...
function FetchNotConsultedDoctors(props) {
  ...
  return (
    <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: '20px' }}>
      {doctors.map((doctors) => (
        <NotConsultedDoctors key={doctors.doctorId} parse_doctor={doctors}  onConsultClick={handleConsultClicktwo} passstatus={props.clickstatus}/>
      ))}
    </Box>
  );
}
